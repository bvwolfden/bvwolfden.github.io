  ...
  return (
    <main>
      ...
      <section className="section history-section" id="history"><div className="shell">
        ...
        <div className="player-table"><p className="table-key"><b>HOW TO READ IT</b> Percentages are earned-point rates: wins plus half of ties, divided by matches played.</p><div className="player-head"><span>BLUE PLAYER</span><span>ALL FORMATS</span><span>PARTNER PLAY</span><span>SINGLES</span><span>CAPTAIN’S READ</span></div>{playerAnalytics.map(([name,all,pairs,singles,read])=><div className="player-row" key={String(name)}><b>{name}</b><span>{all}%</span><span>{pairs}%</span><span className={Number(singles)<40?'low':''}>{singles}%</span><em>{read}</em></div>)}</div>
